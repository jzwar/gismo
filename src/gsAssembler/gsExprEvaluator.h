...
class gsExprEvaluator {
 ...
 public:
  ...
  gsOptionList defaultOptions() {
    ...
    opt.addSwitch("plot.elements",
                  "Include the element mesh in plot (when applicable)", false);
    opt.addSwitch("flipSide",
                  "Flip side of interface where evaluation is performed.",
                  false);
    // opt.addSwitch("plot.cnet", "Include the control net in plot (when
    // applicable)", false);
    return opt;
  }
 ...
 public:
  ...
  expr::gsComposition<T> getVariable(const gsFunctionSet<T> &func,
                                     geometryMap G) {
  ...
  /// Calculates the square root of the lastly computed quantities (eg.
  /// integrals)
  void calcSqrt() {
  ...
  /// Calculates the \a p-th root of the lastly computed quantities (eg.
  /// integrals)
 ...
 public:
  /// Calculates the integral of the expression \a expr on the whole integration
  /// domain
  template <class E>
  ...
  T integralBdr(const expr::_expr<E> &expr) {
    return computeBdr_impl<E, plus_op>(
        expr, m_exprdata->multiBasis().topology().boundaries());
  }
  ...
  T integralInterface(const expr::_expr<E> &expr) {
    return computeInterface_impl<E, plus_op>(
        expr, m_exprdata->multiBasis().topology().interfaces());
  }
  ...
  T maxInterface(const expr::_expr<E> &expr) {
    return computeInterface_impl<E, max_op>(
        expr, m_exprdata->multiBasis().topology().interfaces());
  }
  ...
  T minInterface(const expr::_expr<E> &expr) {
    return computeInterface_impl<E, min_op>(
        expr, m_exprdata->multiBasis().topology().interfaces());
  }
...
#endif
  eval(const expr::_expr<E> &expr, gsGridIterator<T, mode, d> &git,
       const index_t patchInd = 0);

  template <class E, int mode, short_t d>
  typename util::enable_if<!E::ScalarValued, void>::type eval(
      const expr::_expr<E> &expr, gsGridIterator<T, mode, d> &git,
      const index_t patchInd = 0);
  ...
  template <class E>
  typename util::enable_if<!E::ScalarValued, gsAsConstMatrix<T> >::type eval(
      const expr::_expr<E> &testExpr, const gsVector<T> &pt,
      const index_t patchInd = 0);
  ...
  template <class E>
  typename util::enable_if<!E::ScalarValued, gsAsConstMatrix<T> >::type evalIfc(
      const expr::_expr<E> &testExpr, const gsVector<T> &pt,
      const boundaryInterface &ifc);
  ...
  template <class E>
  void testEval(const expr::_expr<E> &expr, const gsVector<T> &pt,
                const index_t patchInd = 0) {
    expr.print(gsInfo);
  ...
  template <class E>
  void interpolate(const expr::_expr<E> &) {
  ...
  template <class E>
  void writeParaview(const expr::_expr<E> &expr, geometryMap G,
                     std::string const &fn) {
 ...
 private:
  ...
  void writeParaview_impl(const expr::_expr<E> &expr, geometryMap G,
                          std::string const &fn);
  ...
  template <class E, class _op>
  T computeInterface_impl(const expr::_expr<E> &expr,
...
T gsExprEvaluator<T>::compute_impl(const expr::_expr<E> &expr) {
  m_value = _op::init();
  m_elWise.clear();
  if (storeElWise) m_elWise.resize(m_exprdata->multiBasis().totalElements());
...
#pragma omp parallel
  {
    ...
    T elVal;
    index_t c = 0;
    for (unsigned patchInd = 0; patchInd < m_exprdata->multiBasis().nBases();
         ++patchInd) {
      // Quadrature rule
      QuRule = gsQuadrature::get(m_exprdata->multiBasis().basis(patchInd),
                                 m_options);
...
#endif
      {
        ...
        // Compute on element
        elVal = _op::init();
        for (index_t k = 0; k != quWeights.rows();
             ++k)  // loop over quad. nodes
...
T gsExprEvaluator<T>::computeBdr_impl(const expr::_expr<E> &expr,
  ...
  for (typename gsBoxTopology::const_biterator bit =  //!! not multipatch!
       ...
       bit != bdrlist.end(); ++bit) {
    // Quadrature rule
    QuRule = gsQuadrature::get(m_exprdata->multiBasis().basis(bit->patch),
                               m_options, bit->direction());

    // Initialize domain element iterator
    typename gsBasis<T>::domainIter domIt =
        m_exprdata->multiBasis()
            .piece(bit->patch)
            .makeDomainIterator(bit->side());
    m_exprdata->getElement().set(*domIt, quWeights);
...
T gsExprEvaluator<T>::computeBdrBc_impl(const bcRefList &BCs,
  ...
  for (typename bcRefList::const_iterator iit = BCs.begin(); iit != BCs.end();
       ++iit) {
    ...
    // Initialize domain element iterator
    typename gsBasis<T>::domainIter domIt = m_exprdata->multiBasis()
                                                .basis(it->patch())
    ...
    for (; domIt->good(); domIt->next()) {
      ...
      if (m_exprdata->points().cols() == 0) continue;
...
template <class T>
template <class E, class _op>
T gsExprEvaluator<T>::computeInterface_impl(const expr::_expr<E> &expr,
                                            const intContainer &iFaces) {
  ...
  for (typename gsBoxTopology::const_iiterator iit = iFaces.begin();
       iit != iFaces.end(); ++iit) {
    ...
    if (iFace.type() == interaction::conforming)
      interfaceMap = gsAffineFunction<T>::make(
          iFace.dirMap(), iFace.dirOrientation(),
          m_exprdata->multiBasis().basis(patch1).support(),
          m_exprdata->multiBasis().basis(patch2).support());
    else
      interfaceMap = gsCPPInterface<T>::make(m_exprdata->multiPatch(),
...
template <class T>
template <class E, int mode, short_t d>
typename util::enable_if<E::ScalarValued, void>::type gsExprEvaluator<T>::eval(
    const expr::_expr<E> &expr, gsGridIterator<T, mode, d> &git,
    const index_t patchInd) {  // to remove
...
//                 static_cast<const
//                 gsGeometry<T>&>(G.source().piece(i)).evaluateMesh(msh);
...
void gsExprEvaluator<T>::writeParaview_impl(const expr::_expr<E> &expr,
  ...
  for (index_t i = 0; i != n; ++i) {
    ...
    gsWriteParaviewTPgrid(gmap ? pts : pt.toMatrix(),  // parameters
                          vals, pt.numPointsCwise(), fileName);

    // Snippet from gsParaviewCollection
    // collection.addPart(fileName+ ".vts");
    GISMO_ASSERT(counter != -1, "Error: collection has been already saved.");
    file << "<DataSet part=\"" << counter++ << "\" file=\"" << fileName
         << ".vts"
         << "\"/>\n";
    // End snippet from gsParaviewCollection
