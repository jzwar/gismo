...
void secDerToHessian(const Eigen::DenseBase<Derived>& secDers,
  ...
  const gsAsConstMatrix<typename Derived::Scalar> ders(secDers.derived().data(),
                                                       sz, secDers.size() / sz);
...
template <class E>
class inv_expr;
template <class E, bool cw = false>
class tr_expr;
template <class E>
class cb_expr;
template <class E>
...
// Call as pow(a,b)
template <class E>
pow_expr<E> pow(_expr<E> const& u, real_t q) {
...
#define Temporary_t                                \
  typename util::conditional<ScalarValued, Scalar, \
...
#define AutoReturn_t \
  typename util::conditional<ScalarValued, real_t, MatExprType>::type
#endif
...
class _expr<E, false> {
 ...
 public:
  ...
  // - Space: 0: not a trial nor a test object (eg. normal vector, force
  // function)
  //          1: a test object  (essentially a right-hand side vector
  //          expression) 2: a trial object 3: a trial+trial object (essentially
  //          a matrix expression)
  ...
  void print(std::ostream& os) const {
    // gsInfo<<"\n Space="<<E::Space<<", ScV="<<E::ScalarValued<<",
    ...
    /*
      ...
      while((pos=tmp.find("gismo::expr::",0))!=std::string::npos)
      tmp.erase(pos,13); while((pos=tmp.find("_expr",0))!=std::string::npos)
      tmp.erase(pos,5); while((pos=tmp.find("<double>",0))!=std::string::npos)
      tmp.erase(pos,8);
      // while((pos=tmp.find("<long double>",0))!=std::string::npos)
      tmp.erase(pos,13);
  ...
  std::ostream& printDetail(std::ostream& os) const {
    os << (isVectorTr() ? "VectorTr "
                        : (isVector() ? "Vector "
                                      : (isMatrix() ? "Matrix " : "Scalar ")))
       << "expression of size "
       << rows()  // bug: this might be invalid if unparsed
       << " x " << cols() << "\n";
    print(os);
    return os;
  }
  ...
  cb_expr<E> cb() const { return cb_expr<E>(static_cast<E const&>(*this)); }
  ...
  mult_expr<real_t, ppart_expr<mult_expr<double, E, false>>, false> npart()
      const {
    return -1 * (-(*this)).ppart();
  }
  ...
  det_expr<E> det() const { return det_expr<E>(static_cast<E const&>(*this)); }
  ...
  max_expr<E> max() const { return max_expr<E>(static_cast<E const&>(*this)); }
  ...
  index_t rows() const { return static_cast<E const&>(*this).rows(); }

  /// Returns the column-size of the expression
  index_t cols() const { return static_cast<E const&>(*this).cols(); }
...
class symbol_expr : public _expr<E> {
 ...
 protected:
  const gsFunctionSet<Scalar>*
      m_fs;  ///< Evaluation source for this FE variable
  const gsFuncData<Scalar>*
      m_fd;         ///< Temporary variable storing flags and evaluation data
  index_t m_d;      ///< Dimension of this (scalar or vector) variable
  bool m_isAcross;  ///< true when this expression is evaluated across an
                    ///< interface
 ...
 public:
  ...
  const gsFuncData<Scalar>& data() const {
    GISMO_ASSERT(NULL != m_fd,
                 "FuncData member not registered " << this << "/" << m_fs);
    return *m_fd;
  }
  ...
  index_t cardinality_impl() const {
    GISMO_ASSERT(this->data().actives.rows() != 0,
...
class gsGeometryMap : public _expr<gsGeometryMap<T>> {
 ...
 public:
  ...
  const gsMapData<T>& data() const {
    GISMO_ASSERT(NULL != m_fd,
                 "gsGeometryMap: invalid data " << m_fs << "," << m_fd);
    return *m_fd;
  }

  index_t targetDim() const { return m_fs->targetDim(); }

  /// Copy the coefficients of another gsGeometryMap to this one, if they are
  /// compatible.
  void copyCoefs(const gsGeometryMap<T>& other) const {
    ...
    const gsMultiPatch<T>& thisMP =
        static_cast<const gsMultiPatch<T>&>(*this->m_fs);
    GISMO_ASSERT(dynamic_cast<const gsMultiPatch<T>*>(other.m_fs), "error");
    const gsMultiPatch<T>& otherMP =
        static_cast<const gsMultiPatch<T>&>(*other.m_fs);
  ...
  void deformBy(const gsFeSolution<T>& displacement) const {
    ...
    const gsMultiBasis<T>& mb =
        static_cast<const gsMultiBasis<T>&>(displacement.space().source());
    const gsMultiPatch<T>& mp =
        static_cast<const gsMultiPatch<T>&>(*this->m_fs);
    GISMO_ASSERT(
        dynamic_cast<const gsMultiBasis<T>*>(&displacement.space().source()),
        "error");
    GISMO_ASSERT(dynamic_cast<const gsMultiPatch<T>*>(this->m_fs), "error");
    ...
    for (size_t p = 0; p < mp.nPatches(); p++) {
      ...
      for (index_t c = 0; c != dim; c++) {
        ...
        for (index_t i = 0; i < sz; ++i) {
          ...
          if (displacement.mapper().is_free_index(
                  ii))  // DoF value is in the defVector
          {
            result(i, c) += displacement.coefs().at(ii);
          } else {
            result(i, c) += displacement.fixedPart().at(
                displacement.mapper().global_to_bindex(ii));
          }
        }
      }
    }
...
class gsFeElement {
 ...
 public:
  ...
  AreaRetType area() const { return integral(_expr<T, true>(1)); }
...
class integral_expr : public _expr<integral_expr<E>> {
 ...
 public:
  ...
  void parse(gsExprHelper<Scalar>& evList) const { _ff.parse(evList); }
...
template <class T>
class gsComposition : public symbol_expr<gsComposition<T>> {  // comp(f,G)
  friend class gismo::gsExprHelper<T>;
 ...
 public:
  ...
  typename gsMatrix<T>::constColumn eval(const index_t k) const {
...
class gsFeSpace : public symbol_expr<gsFeSpace<T>> {
 ...
 public:
  ...
  void getCoeffs(const gsMatrix<T>& solVector, gsMatrix<T>& result,
    ...
    const gsMultiBasis<T>& mb =
        static_cast<const gsMultiBasis<T>&>(this->source());
    GISMO_ASSERT(dynamic_cast<const gsMultiBasis<T>*>(&this->source()),
                 "error");
      ...
      for (index_t i = 0; i < sz; ++i) {
        ...
        if (m_sd->mapper.is_free_index(ii))  // DoF value is in the solVector
          for (index_t s = 0; s != solVector.cols(); ++s)
            result(i, c + s) =
                solVector(ii, s);  // assume dim==1 xor solVector.cols()==1
  ...
  void setupMapper(gsDofMapper dofsMapper) const {
    GISMO_ASSERT(dofsMapper.isFinalized(),
                 "The provided dof-mapper is not finalized.");
    GISMO_ASSERT(
        dofsMapper.mapSize() == static_cast<size_t>(this->source().size() *
                                                    dofsMapper.numComponents()),
        "The dof-mapper is not consistent: mapSize()="
            << dofsMapper.mapSize()
    ...
    if (mb != nullptr) {
      ...
      for (typename gsBoundaryConditions<T>::const_iterator it =
        ...
        GISMO_ASSERT(
            ...
            "Problem: a boundary condition is set on a patch id which does not "
            "exist.");
      ...
      for (typename gsBoundaryConditions<T>::const_iterator it =
        ...
        if (!it->ps.parameter()) bnd.swap(bnd1);
        for (index_t c = 0; c != dim; c++)  // for all components
        {
          if (c == cc || cc == -1)
            for (index_t k = 0; k < bnd.size(); ++k)
              m_sd->mapper.matchDof(it->ps.patch, (bnd)(k, 0), it->ps.patch,
                                    (bnd1)(k, 0), c);
        }
      }
        ...
        for (index_t c = 0; c != dim; c++)  // for all components
        {
          if (c == cc || cc == -1)
            for (index_t k = 0; k < bnd.size() - 1; ++k)
              m_sd->mapper.matchDof(it->ps.patch, (bnd)(0, 0), it->ps.patch,
                                    (bnd)(k + 1, 0), c);
        }
      }

      // corners
      for (typename gsBoundaryConditions<T>::const_citerator it =
               bc.cornerBegin();
           it != bc.cornerEnd(); ++it) {
        for (index_t r = 0; r != this->dim(); ++r) {
          ...
          GISMO_ASSERT(static_cast<size_t>(it->patch) < mb->nBases(),
                       "Problem: a corner boundary condition is set on a patch "
                       "id which does not exist.");
          m_sd->mapper.eliminateDof(
              mb->basis(it->patch).functionAtCorner(it->corner), it->patch,
              it->component);
        }
      }
      ...
      for (typename gsBoundaryConditions<T>::const_iterator it =
        ...
        GISMO_ASSERT(it->ps.patch == 0,
                     "Problem: a boundary condition is set on a patch id which "
                     "does not exist.");
      ...
      if (0 == this->interfaceCont())  // C^0 matching interface
      {
        ...
        for (gsBoxTopology::const_iiterator it = mapb->getTopol().iBegin();
          ...
          int1 = mapb->basis(it->first().patch)
                     .boundaryOffset(it->first().side(), 0);
          int2 = mapb->basis(it->second().patch)
                     .boundaryOffset(it->second().side(), 0);

          m_sd->mapper.matchDofs(it->first().patch, int1, it->second().patch,
                                 int2);
        }
      }
      ...
      {
        GISMO_ERROR(
            "Boundary offset function is not implemented for gsMappedBasis in "
            "general.");
      }
      ...
      for (typename gsBoundaryConditions<T>::const_citerator it =
        ...
        GISMO_ASSERT(it->patch < mapb->nPieces(),
                     ...
                     "id which does not exist.");
        m_sd->mapper.eliminateDof(
            mapb->basis(it->patch).functionAtCorner(it->corner), it->patch,
            it->component);
      }
    } else {
...
template <class T>
inline bool operator==(const gsFeSpace<T>& a, const gsFeSpace<T>& b) {
...
class gsFeSolution : public _expr<gsFeSolution<T>> {
 ...
 public:
  ...
  const gsMatrix<T>& eval(index_t k) const {
    ...
    GISMO_ASSERT(_Sv->size() == map.freeSize(),
                 "The solution vector has wrong dimensions: "
                     << _Sv->size() << " != " << map.freeSize());

    for (index_t c = 0; c != _u.dim(); c++)  // for all components
    {
      for (index_t i = 0; i != _u.data().actives.size(); ++i) {
        const index_t ii =
            map.index(_u.data().actives.at(i), _u.data().patchId, c);
 ...
 public:
  ...
  index_t parDim() const { return _u.source().domainDim(); }
  ...
  void setSolutionVector(gsMatrix<T>& solVector) { _Sv = &solVector; }
  ...
  /// @param patch The index of the patch whose coefficients will be set. By
  /// default all patches are affected,
      ...
      for (index_t c = 0; c != dim; c++)  // for all components
      {
        ...
        for (index_t i = 0; i < sz; ++i) {
          ...
          if (_u.mapper().is_free_index(ii))  // DoF value is in the solVector
          {
            ...
            result(i + offset, 0) =
                _u.fixedPart().at(_u.mapper().global_to_bindex(ii));
        }
        offset += sz;
      }
    }
  ...
  void extract(gsMultiPatch<T>& result) const {
    ...
    if (const gsMultiBasis<T>* basis =
            dynamic_cast<const gsMultiBasis<T>*>(&_u.source()))
  ...
  memory::unique_ptr<gsGeometry<T>> extractPiece(const index_t p) const {
    if (const gsBasis<T>* b =
            dynamic_cast<const gsBasis<T>*>(&_u.source().piece(p))) {
      gsMatrix<T> cf;
      extract(cf, p);
...
class tr_expr : public _expr<tr_expr<E, cw>> {
 ...
 public:
  ...
  tr_expr(_expr<E> const& u) : _u(u) {}

 public:
  enum { ColBlocks = E::ColBlocks, ScalarValued = E::ScalarValued };
  enum {
    Space = cw ? E::Space : (E::Space == 1 ? 2 : (E::Space == 2 ? 1 : E::Space))
  };
 ...
 private:
  /*
    ...
    eval_impl(const U k, typename util::enable_if<1==ColBlocks,U>::type* =
    nullptr) { return _u.eval(k).blockTranspose(_u.cols()/_u.rows()); }

    template<class U> EIGEN_STRONG_INLINE MatExprType
    eval_impl(const U k, typename util::enable_if<0==ColBlocks,U>::type* =
    nullptr) { return _u.eval(k).transpose(); }
  */
};
 ...
 public:
  ...
  const gsMatrix<Scalar>& eval(const index_t k) const {
    ...
    if (E::ColBlocks) {
      ...
      GISMO_ASSERT(res.rows() % _u.rows() == 0 && res.cols() % _u.cols() == 0,
                   "Result is not a multiple of the space dimensions?");

      index_t cardinality;
      if ((cardinality = res.rows() / _u.rows()) >= 1 &&
          res.cols() / _u.cols() == 1)  // stored in rows
      {
        res.resize(_u.rows(), cardinality * _u.cols());
        for (index_t r = 0; r != cardinality; r++)
          res.block(0, r * _u.cols(), _u.rows(), _u.cols()) =
              ev.block(r * _u.rows(), 0, _u.rows(), _u.cols());
      } else if ((cardinality = res.rows() / _u.rows()) == 1 &&
                 res.cols() / _u.cols() >= 1)  // stored in cols ----->>>> This
                                               // is already colBlocks???
      {
        res.resize(_u.rows(), cardinality * _u.cols());
        for (index_t r = 0; r != cardinality; r++)
          res.block(0, r * _u.cols(), _u.rows(), _u.cols()) =
              ev.block(0, r * _u.cols(), _u.rows(), _u.cols());
      }
    }
    return res;
  }
...
class temp_expr : public _expr<temp_expr<E>> {
 ...
 public:
  temp_expr(_expr<E> const& u) : _u(u) {}
...
class trace_expr : public _expr<trace_expr<E>> {
 ...
 public:
  trace_expr(_expr<E> const& u) : _u(u) {
    ...
    // GISMO_ASSERT(0== _u.cols()%_u.rows(), "Expecting square-block expression,
    // got " << _u.rows() <<" x "<< _u.cols() );
  }
...
class reshape_expr : public _expr<reshape_expr<E>> {
 ...
 public:
  ...
  const gsAsConstMatrix<Scalar> eval(const index_t k) const {
    ...
    GISMO_ASSERT(_u.rows() * _u.cols() == _n * _m,
                 "Wrong dimension " << _u.rows() << " x " << _u.cols()
                                    << "!=" << _n << " * " << _m);
    tmp = _u.eval(k);
...
template <typename E>
EIGEN_STRONG_INLINE reshape_expr<E> const reshape(E const& u, index_t n,
                                                  index_t m) {
...
class replicate_expr : public _expr<replicate_expr<E>> {
 ...
 public:
  ...
  replicate_expr(_expr<E> const& u, index_t n, index_t m)
      : _u(u), _n(n), _m(m) {}
...
template <typename E>
EIGEN_STRONG_INLINE replicate_expr<E> const replicate(E const& u, index_t n,
                                                      index_t m = 1) {
  return replicate_expr<E>(u, n, m);
}

/**
   Transforms a matrix expression into a vector expression by computing the
   vector [ a b c+d]^T for each matrix block [ a d ] [ c b ]
*/
template <class E>
...
class diag_expr : public _expr<diag_expr<E>> {
 ...
 public:
  diag_expr(_expr<E> const& u) : _u(u) {
    GISMO_ASSERT(0 == _u.cols() % _u.rows(),
                 "Expecting square-block expression, got " << _u.rows() << " x "
                                                           << _u.cols());
  }
...
#define GISMO_EXPR_VECTOR_EXPRESSION(name, mname, isSv)                       \
  template <class E>                                                          \
  class name##_##expr : public _expr<name##_##expr<E>> {                      \
    typename E::Nested_t _u;                                                  \
                                                                              \
   public:                                                                    \
    typedef typename E::Scalar Scalar;                                        \
...
class max_expr : public _expr<max_expr<E>> {
 ...
 private:
  template <class U>
  inline typename util::enable_if<util::is_same<U, gsFeSpace<Scalar>>::value,
...
class constMat_expr : public _expr<constMat_expr> {
 ...
 public:
  gsMatrix<Scalar> eval(const index_t) const { return _mat; }
...
class sign_expr : public _expr<sign_expr<E>> {
 ...
 public:
  ...
  sign_expr(_expr<E> const& u, Scalar tolerance = 0.0)
      : _u(u), _tol(tolerance) {
    GISMO_ASSERT(_tol >= 0,
  ...
  void parse(gsExprHelper<Scalar>& el) const { _u.parse(el); }
...
class value_expr : public _expr<value_expr<E>> {
 ...
 private:
  template <class U>
  static inline typename util::enable_if<U::ScalarValued, Scalar>::type
...
template <class E>
class neg_expr : public _expr<neg_expr<E>> {
  typename E::Nested_t _u;

 public:
  typedef typename E::Scalar Scalar;
  explicit neg_expr(_expr<E> const& u) : _u(u) {}
...
class grad_expr : public _expr<grad_expr<E>> {
 ...
 public:
  ...
  const gsMatrix<Scalar>& eval(const index_t k) const {
    ...
    tmp = _u.data()
              .values[1]
              .reshapeCol(k, cols(), cardinality_impl())
              .transpose();
    return tmp;
  }
  ...
  const gsMatrix<T>& eval(index_t k) const {
    ...
    for (index_t c = 0; c != _u.dim(); c++) {
      for (index_t i = 0; i != _u.data().actives.size(); ++i) {
        ...
        {
          res.row(c) +=
              _u.coefs().at(ii) * _u.data()
                                      .values[1]
                                      .col(k)
                                      .segment(i * _u.parDim(), _u.parDim())
                                      .transpose();
        } else {
  ...
  */
  ...
  const gsMatrix<Scalar>& eval(const index_t k) const {
    ...
    for (index_t i = 0; i != d; ++i)
      res.col(i).segment(i * n, n) =
          u.data().values[1].reshapeCol(k, d, n).row(i);
    return res;
  }
  ...
  const gsFeSpace<T>& colVar() const { return gsNullExpr<Scalar>::get(); }
...
/*
  Expression for the nabla2 (\f$\nabla^2\f$ or Del2) of a finite element
  variable, see also https://en.wikipedia.org/wiki/Del
...
class nabla2_expr : public _expr<nabla2_expr<T>> {
  ...
  */
  ...
  nabla2_expr(const gsFeVariable<T>& _u) : u(_u) {}
 ...
 public:
  ...
  const gsVector<Scalar>& eval(const index_t k) const {
    if (_G.targetDim() == 2) {
      ...
      res.resize(3);
      res.col3d(0) =
          _G.data().normals.col3d(k).cross(_G.data().outNormals.col3d(k));
      return res;
    } else
  ...
  const gsMatrix<T>& eval(const index_t k) const {
    ...
    for (index_t c = 0; c != _u.dim(); c++)
      for (index_t i = 0; i != numActs; ++i) {
        const index_t ii =
            map.index(_u.data().actives.at(i), _u.data().patchId, c);
        deriv2 = _u.data().values[2].block(
            i * numDers, k, _u.parDim(),
            1);  // this only takes d11, d22, d33 part. For all the derivatives
                 // [d11, d22, d33, d12, d13, d23]:
                 // col.block(i*numDers,k,numDers,1)
        if (map.is_free_index(ii))  // DoF value is in the solVector
          res.at(c) += _u.coefs().at(ii) * deriv2.sum();
        else
          res.at(c) +=
              _u.fixedPart().at(map.global_to_bindex(ii)) * deriv2.sum();
      }
    return res;
  }
...
class fform2nd_expr : public _expr<fform2nd_expr<T>> {
 ...
 public:
  ...
  const gsAsConstMatrix<Scalar> eval(const index_t k) const {
    return gsAsConstMatrix<Scalar>(_G.data().fundForms.col(k).data(), rows(),
                                   cols());
  }
...
class jacInv_expr : public _expr<jacInv_expr<T>> {
 ...
 public:
  ...
  jacInv_expr(const gsGeometryMap<T>& G) : _G(G) {
    // Note: for non-square Jacobian matrices, generalized inverse, i.e.: (J^t
    // J)^{-t} J^t
...
class jac_expr : public _expr<jac_expr<E>> {
 ...
 public:
  ...
  MatExprType eval(const index_t k) const {
    if (0 != Space) {
      ...
      res = _u.data()
                .values[1]
                .reshapeCol(k, _u.parDim(), _u.targetDim())
                .transpose()
                .blockDiag(_u.dim());
    }
    return res;
 ...
 private:
  ...
  template <class U>
  inline typename util::enable_if<!(util::is_same<U, gsFeSpace<Scalar>>::value),
                                  index_t>::type  // What about solution??
...
class jac_expr<gsGeometryMap<T>> : public _expr<jac_expr<gsGeometryMap<T>>> {
 ...
 public:
  ...
  MatExprType eval(const index_t k) const {
    ...
    return _G.data()
        .values[1]
        .reshapeCol(k, _G.data().dim.first, _G.data().dim.second)
        .transpose();
  }
...
class hess_expr<gsFeSolution<T>> : public _expr<hess_expr<gsFeSolution<T>>> {
 ...
 public:
  ...
  mutable gsMatrix<T> res;
  const gsMatrix<T>& eval(const index_t k) const {
    GISMO_ASSERT(1 == _u.data().actives.cols(),
    ...
    if (1 == _u.dim()) {
      ...
      for (index_t i = 0; i != numActs; ++i) {
        const index_t ii =
            map.index(_u.data().actives.at(i), _u.data().patchId, 0);
    ...
    // In the vector case, the hessian is returned as a matrix where each row
    // corresponds to the component of the solution and contains the derivatives
    // in the columns
    else {
      ...
      for (index_t c = 0; c != _u.dim(); c++)
        for (index_t i = 0; i != numActs; ++i) {
          ...
          deriv2 = _u.space()
                       .data()
                       .values[2]
                       .block(i * numDers, k, numDers,
                              1)
                       .transpose();  // start row, start col, rows, cols
          if (map.is_free_index(ii))  // DoF value is in the solVector
...
class dJacG_expr : public _expr<dJacG_expr<T>> {
 ...
 public:
  ...
  MatExprType eval(const index_t k) const {
    ...
    const index_t s =
        _G.data().derivSize();  // dim.first*(_G.data().dim.first+1)/2;
    (void)s;
...
class mult_expr<E1, E2, false> : public _expr<mult_expr<E1, E2, false>> {
 ...
 public:
  enum {
    ScalarValued = E1::ScalarValued && E2::ScalarValued,
  ...
  mult_expr(_expr<E1> const& u, _expr<E2> const& v) : _u(u), _v(v) {}

  mutable Temporary_t tmp;
  const Temporary_t& eval(const index_t k) const {
    GISMO_ASSERT(0 == _u.cols() * _v.rows() || _u.cols() == _v.rows(),
                 "Wrong dimensions " << _u.cols() << "!=" << _v.rows()
                                     << " in * operation:\n"
                                     << _u << " times \n"
                                     << _v);
...
class mult_expr<E1, E2, true> : public _expr<mult_expr<E1, E2, true>> {
 ...
 public:
  ...
  const gsMatrix<Scalar>& eval(const index_t k) const {
    ...
    {
      res.resize(ur, vc * nb);
      GISMO_ASSERT(tmpA.cols() == uc * nb,
                   "Dimension error.. " << tmpA.cols() << "!=" << uc * nb);
...
// template <typename E> class scmult_expr : public _expr<scmult_expr<E> >
{
 ...
 public:
  ...
  mult_expr(Scalar const& c, _expr<E2> const& v) : _c(c), _v(v) {}
  ...
  void parse(gsExprHelper<Scalar>& evList) const { _v.parse(evList); }
...
class collapse_expr : public _expr<collapse_expr<E1, E2>> {
 ...
 public:
  ...
  collapse_expr(_expr<E1> const& u, _expr<E2> const& v) : _u(u), _v(v) {}
  ...
  const gsFeSpace<Scalar>& colVar() const { GISMO_ERROR("none"); }
...
class frprod_expr : public _expr<frprod_expr<E1, E2>> {
 ...
 public:
  frprod_expr(_expr<E1> const& u, _expr<E2> const& v) : _u(u), _v(v) {
    // todo: add check() functions, which will evaluate expressions on an empty
    // matrix (no points) to setup initial dimensions ??? GISMO_ASSERT(_u.rows()
    // == _v.rows(),
    //              "Wrong dimensions "<<_u.rows()<<"!="<<_v.rows()<<" in %
    //              operation");
    // GISMO_ASSERT(_u.cols() == _v.cols(),
    //              "Wrong dimensions "<<_u.cols()<<"!="<<_v.cols()<<" in %
    //              operation");
  }

  const gsMatrix<Scalar>& eval(
      const index_t k) const  // todo: specialize for nb==1
  {
    ...
    for (index_t i = 0; i != nb; ++i)  // all with all
      for (index_t j = 0; j != nb; ++j)
        res(i, j) = (A.middleCols(i * rb, rb).array() *
                     B.middleCols(j * rb, rb).array())
                        .sum();
    return res;
  }
...
class frprod_expr<E1, E2, false> : public _expr<frprod_expr<E1, E2, false>> {
 ...
 public:
      ...
      const index_t k) const  // todo: specialize for nb==1
  {
    ...
    res.resize(nb, 1);
    for (index_t i = 0; i != nb; ++i) {  // all with all
      res(i, 0) = (A.middleCols(i * rb, rb).array() * B.array()).sum();
    }
    return res;
  }
...
class divide_expr : public _expr<divide_expr<E1, E2>> {
 ...
 public:
  ...
  divide_expr(_expr<E1> const& u, _expr<E2> const& v) : _u(u), _v(v) {
    GISMO_STATIC_ASSERT(E2::ScalarValued,
                        "The denominator needs to be scalar valued.");
  }

  AutoReturn_t eval(const index_t k) const { return (_u.eval(k) / _v.eval(k)); }
...
class divide_expr<E1, typename E1::Scalar>
 ...
 public:
  ...
  divide_expr(_expr<E1> const& u, Scalar const c) : _u(u), _c(c) {}

  AutoReturn_t eval(const index_t k) const { return (_u.eval(k) / _c); }
...
class divide_expr<typename E2::Scalar, E2>
 ...
 public:
  ...
  divide_expr(Scalar const c, _expr<E2> const& u) : _c(c), _u(u) {
    GISMO_STATIC_ASSERT(E2::ScalarValued,
                        "The denominator needs to be scalar valued.");
  }

  Scalar eval(const index_t k) const { return (_c / _u.val().eval(k)); }
...
class add_expr : public _expr<add_expr<E1, E2>> {
 ...
 public:
  ...
  add_expr(_expr<E1> const& u, _expr<E2> const& v) : _u(u), _v(v) {
    GISMO_ENSURE((int)E1::Space == (int)E2::Space &&
                 ...
                 " namely:\n"
                     << _u << "\n"
                     << _v << " \nvars:\n"
                     << _u.rowVar().id() << "!=" << _v.rowVar().id() << ", "
                     << _u.colVar().id() << "!=" << _v.colVar().id()
                     << " \nspaces:\n"
...
class summ_expr : public _expr<summ_expr<E1, E2>> {
 public:
  ...
  const gsMatrix<Scalar>& eval(const index_t k) const {
    ...
    GISMO_ASSERT(_M.cols() == _M.rows(),
                 "Matrix must be square: " << _M.rows() << " x " << _M.cols()
                                           << " expr: " << _M);
    GISMO_ASSERT(mb == _u.cols(), "cardinality must match vector, but card(M)="
                                      << _M.cardinality()
                                      << " and cols(u)=" << _u.cols());
...
class sub_expr : public _expr<sub_expr<E1, E2>> {
 ...
 public:
  ...
  sub_expr(_expr<E1> const& u, _expr<E2> const& v) : _u(u), _v(v) {
    GISMO_ENSURE((int)E1::Space == (int)E2::Space &&
                     _u.rowVar() == _v.rowVar() && _u.colVar() == _v.colVar(),
  ...
  mutable Temporary_t res;
  const Temporary_t& eval(const index_t k) const {
    // GISMO_ASSERT(_u.rowVar().id()==_v.rowVar().id() &&
    // _u.rowVar().isAcross()==_v.rowVar().isAcross(),
    //     "The row spaces are not split compatibly.");
    // GISMO_ASSERT(_u.colVar().id()==_v.colVar().id() &&
    // _u.colVar().isAcross()==_v.colVar().isAcross(),
    ...
    GISMO_ASSERT(_u.cols() == _v.cols(), "Wrong dimensions "
                                             ...
                                             << _v);
    GISMO_ASSERT(
        _u.cardinality() == _u.cardinality(),
        "Cardinality " << _u.cardinality() << " != " << _v.cardinality());
    // return (_u.eval(k) - _v.eval(k) ).eval();
    // return (_u.eval(k) - _v.eval(k) ); // any temporary matrices eval(.) will
    // leak mem.
    res = _u.eval(k) - _v.eval(k);
    return res;
  }
...
class symm_expr : public _expr<symm_expr<E>> {
 ...
 public:
  ...
  enum {
    Space = (0 == E::Space ? 0 : E::Space),
    ScalarValued = E::ScalarValued,
    ColBlocks = E::ColBlocks
  };

  symm_expr(_expr<E> const& u) : _u(u) {}
...
class symmetrize_expr : public _expr<symmetrize_expr<E>> {
 ...
 public:
  ...
  symmetrize_expr(_expr<E> const& u) : _u(u) {}
...
EIGEN_STRONG_INLINE constMat_expr mat(const gsMatrix<real_t> mat) {
...
EIGEN_STRONG_INLINE onormal_expr<T> nv(const gsGeometryMap<T>& u) {
  return onormal_expr<T>(u);
}

template <class T>
EIGEN_STRONG_INLINE normal_expr<T> sn(const gsGeometryMap<T>& u) {
...
EIGEN_STRONG_INLINE tangent_expr<T> tv(const gsGeometryMap<T>& u) {
  return tangent_expr<T>(u);
}

template <class E>
EIGEN_STRONG_INLINE lapl_expr<E> lapl(const symbol_expr<E>& u) {
  return lapl_expr<E>(u);
}

template <class T>
EIGEN_STRONG_INLINE lapl_expr<gsFeSolution<T>> lapl(const gsFeSolution<T>& u) {
...
template <class E>
EIGEN_STRONG_INLINE hess_expr<E> hess(const symbol_expr<E>& u) {
  return hess_expr<E>(u);
}

/// The hessian of a geometry map
template <class T>
EIGEN_STRONG_INLINE hess_expr<gsGeometryMap<T>> hess(
...
template <typename E1, typename E2>
EIGEN_STRONG_INLINE mult_expr<E1, E2> const operator*(_expr<E1> const& u,
                                                      _expr<E2> const& v) {
  return mult_expr<E1, E2>(u, v);
}

template <typename E2>
EIGEN_STRONG_INLINE mult_expr<typename E2::Scalar, E2, false> const operator*(
    typename E2::Scalar const& u, _expr<E2> const& v) {
  return mult_expr<typename E2::Scalar, E2, false>(u, v);
}

template <typename E1>
EIGEN_STRONG_INLINE mult_expr<typename E1::Scalar, E1, false> const operator*(
    _expr<E1> const& v, typename E1::Scalar const& u) {
  return mult_expr<typename E1::Scalar, E1, false>(u, v);
}

template <typename E1>
EIGEN_STRONG_INLINE neg_expr<E1> const operator-(_expr<E1> const& u) {
  return neg_expr<E1>(u);
}

template <typename E>
mult_expr<constMat_expr, E> const operator*(
    gsMatrix<typename E::Scalar> const& u, _expr<E> const& v) {
  return mult_expr<constMat_expr, E>(mat(u), v);
}

template <typename E>
mult_expr<E, constMat_expr> const operator*(
    _expr<E> const& u, gsMatrix<typename E::Scalar> const& v) {
  return mult_expr<E, constMat_expr>(u, mat(v));
}

/// Frobenious product (also known as double dot product) operator for
/// expressions
template <typename E1, typename E2>
EIGEN_STRONG_INLINE frprod_expr<E1, E2> const operator%(_expr<E1> const& u,
                                                        _expr<E2> const& v) {
...
template <typename E1, typename E2>
EIGEN_STRONG_INLINE divide_expr<E1, E2> const operator/(_expr<E1> const& u,
                                                        _expr<E2> const& v) {
  return divide_expr<E1, E2>(u, v);
}

template <typename E>
EIGEN_STRONG_INLINE divide_expr<E, typename E::Scalar> const operator/(
    _expr<E> const& u, const typename E::Scalar v) {
  return divide_expr<E, typename E::Scalar>(u, v);
}

template <typename E>
EIGEN_STRONG_INLINE divide_expr<typename E::Scalar, E> const operator/(
    const typename E::Scalar u, _expr<E> const& v) {
  return divide_expr<typename E::Scalar, E>(u, v);
}

/// Addition operator for expressions
template <typename E1, typename E2>
EIGEN_STRONG_INLINE add_expr<E1, E2> const operator+(_expr<E1> const& u,
                                                     _expr<E2> const& v) {
...
template <typename E>
EIGEN_STRONG_INLINE add_expr<E, _expr<typename E::Scalar, true>> operator+(
    _expr<E> const& u, const typename E::Scalar v) {
  return add_expr<E, _expr<typename E::Scalar>>(
      u, _expr<typename E::Scalar, true>(v));
}

/// Addition operator for expressions and numbers
template <typename E>
EIGEN_STRONG_INLINE add_expr<E, _expr<typename E::Scalar, true>> operator+(
    const typename E::Scalar v, _expr<E> const& u) {
  return add_expr<E, _expr<typename E::Scalar>>(
      u, _expr<typename E::Scalar, true>(v));
}
...
/// Matrix by space TODO: find better name and/or description? And is this the
/// best place? [Jg Jg Jg] * Jb ..
template <typename E1, typename E2>
EIGEN_STRONG_INLINE matrix_by_space_expr<E1, E2> const matrix_by_space(
...
/// best place? [Jg Jg Jg] * Jb ..
template <typename E1, typename E2>
EIGEN_STRONG_INLINE matrix_by_space_expr_tr<E1, E2> const matrix_by_space_tr(
    E1 const& u, E2 const& v) {
...
template <typename E1, typename E2>
EIGEN_STRONG_INLINE sub_expr<E1, E2> const operator-(_expr<E1> const& u,
                                                     _expr<E2> const& v) {
  return sub_expr<E1, E2>(u, v);
}

template <typename E2>
EIGEN_STRONG_INLINE sub_expr<_expr<typename E2::Scalar>, E2> const operator-(
    typename E2::Scalar const& s, _expr<E2> const& v) {
  // assert E2::ScalarValued
  return sub_expr<_expr<typename E2::Scalar>, E2>(_expr<typename E2::Scalar>(s),
                                                  v);
}
...
#define GISMO_SHORTCUT_VAR_EXPRESSION(name, impl)             \
  template <class E>                                          \
  EIGEN_STRONG_INLINE auto name(const E& u)->decltype(impl) { \
    return impl;                                              \
  }
#define GISMO_SHORTCUT_MAP_EXPRESSION(name, impl)                            \
  template <class T>                                                         \
  EIGEN_STRONG_INLINE auto name(const gsGeometryMap<T>& G)->decltype(impl) { \
    return impl;                                                             \
  }
#define GISMO_SHORTCUT_PHY_EXPRESSION(name, impl)                           \
  template <class E>                                                        \
  EIGEN_STRONG_INLINE auto name(const E& u,                                 \
...
GISMO_SHORTCUT_PHY_EXPRESSION(igrad,
                              grad(u) *
                                  jac(G).ginv())  // transpose() problem ??
GISMO_SHORTCUT_VAR_EXPRESSION(igrad,
                              grad(u))  // u is presumed to be defined over G
